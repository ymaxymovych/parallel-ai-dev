...
if [ -e "$PROJECT_MAP" ]; then
  # «Заповнена» = є ≥2 змістовні рядки (не заголовки, не коменти, не роздільники
  # таблиць). У чистому шаблоні змістовний рядок один — шапка таблиці; будь-який
  # реальний запис (рядок таблиці АБО прозовий опис — карти бувають і такі)
  # дає другий. Рахуємо зміст, а не формат.
  R6_N=$(strip_noise "$PROJECT_MAP" | grep -v '^#' | grep -v '^[[:space:]]*|[-: |]*$' \
         | grep -c '[[:alnum:]]' || true)
  [ "${R6_N:-0}" -lt 2 ] && rfail R6 "$(basename "$PROJECT_MAP") порожній — впиши, що в проєкті ВЖЕ існує (це насіння анти-кіл: без карти чати будуватимуть уже збудоване)"
fi
...
if [ "$FULL" = 1 ] || [ "$HAS_CONF" = 1 ]; then
  ...
  if [ -f "$WORKSTREAMS" ]; then
    ...
    WS_ROWS=$(sed '/<!--/,/-->/d' "$WORKSTREAMS" | grep '^[[:space:]]*|' | grep '\[COORD\]' || true)
